import { auth, db, setDoc, doc, getDoc, collection, query, where, getDocs, onSnapshot, deleteDoc, signInWithPopup, GoogleAuthProvider, onAuthStateChanged, signOut, or } from "./firebase-config.js";
    ...
    function populateSelects() {
      ...
      // Populate team-member role select
      const teRole = document.getElementById('te-role');
      if (teRole) {
        const currentVal = teRole.value;
        const roles = (state.activeOrg && state.activeOrg.roles) ? state.activeOrg.roles : ['Owner', 'Admin', 'Member'];
        teRole.innerHTML = roles.map(r => `<option value="${escapeHtml(r)}">${escapeHtml(r)}</option>`).join('');
        if (currentVal) teRole.value = currentVal;
      }
      ...
      // Re-render custom roles list in settings
      renderCustomRoles();
    }
    ...
    window.saveClient = async () => {
      try {
        ...
        showToast(isNew ? 'Client created' : 'Client updated');
      } catch (e) {
        showToast('Error: ' + e.message);
